...
function runlang(t: string, inp: string) {
  input_cnt = 0;
  try {
    let a = inp.split("\n\n");
    let o = "";
...
function actualRunlang(t: string, inp: string) {
  inp = inp || "";
  let stack = [];
  let new_inp = [];
  if (inp !== "") {
    for (let i = 0; i < inp.split("\n")
      .length; i++) {
      if (!!Number(inp.split("\n")[i]) || inp.split("\n")[i] === "0") {
  ...
  for (let i = 0; i < t.length; i++) {
    if (stringmode) {
      ...
      let b;
      let c;
      instructionSwitch: switch (t[i]) {
        case "☺":
          sPop(stack, new_inp);
          break instructionSwitch;
        case "☻":
          sPop(stack, new_inp);
          sPop(stack, new_inp);
          break instructionSwitch;
